  ...
  const readMsg=useCallback(async (cid,mid)=>{
    ...
    if (targetMsg.ciphertexts && session) {
      ...
      if (myCipher) {
         ...
         // ⏳ SYNC SUPABASE: Marquer le message comme "lu" + déclencher l'expiration réelle (TTL = 3 min)
         const expiresAt = new Date(Date.now() + 180 * 1000).toISOString();
         supabase.from('messages').update({ read_at: new Date().toISOString(), expires_at: expiresAt }).eq('id', mid).then(({error})=>{
           if(error) console.error("Erreur Sync Vanish:", error);
           else console.log(`[TTL] Message ${mid} expirera à ${expiresAt}`);
         });
    ...
    <div style={{
      ...
      <div style={{height:56,background:C.s1,borderTop:`1px solid ${C.border}`,display:'flex',alignItems:'stretch',flexShrink:0}}>
        {[
          ...
          {id:'logout',label:'Log Out',dot:false,icon:a=><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={C.red} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>},
        ].map(t=>{
          const a=tab===t.id;
          return (
            <button key={t.id} onClick={async ()=>{
              ...
              setTab(t.id);
              if(t.id!=='chats') setActiveConv(null);
            }} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:4,cursor:'pointer',border:'none',background:'transparent',position:'relative',padding:0}}>
