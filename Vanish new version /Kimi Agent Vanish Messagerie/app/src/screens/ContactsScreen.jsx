  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-4 py-3 flex-shrink-0">
        <div className="flex items-center gap-3 px-4 py-3 rounded-2xl backdrop-blur-xl bg-black/40 border border-white/10 shadow-inner focus-within:bg-white/10 focus-within:border-white/20 transition-all">
              ...
              {grouped[letter].map((contact, i) => (
                <div
                  ...
                  className={`flex items-center gap-3 px-4 py-3.5 transition-colors hover:bg-white/5 ${
                    i < grouped[letter].length - 1 ? 'border-b border-white/5' : ''
                  }`}
                >
                  <button
                    onClick={() => setSelectedContact(contact)}
                    className="flex items-center gap-3.5 flex-1 min-w-0 text-left active:scale-[0.98] transition-transform"
                  >
                    <Av name={contact.name} size={44} online={contact.online} />
                    <div className="flex-1 min-w-0">
                      <p className="text-[15px] font-medium text-white/90 truncate tracking-wide">{contact.name}</p>
                      <p className="text-[12px] font-mono mt-0.5" style={{ color: 'rgba(255,255,255,0.4)' }}>
                        {contact.phone}
                      </p>
                    </div>
                  </button>
